...
func _ready():
	...
	match Global.language:
		Global.LANGUAGE.EN:
			NotificationCenter.notify("DODGE THE BOMBS!")
		Global.LANGUAGE.PT:
			NotificationCenter.notify("ESQUIVE DAS BOMBAS!")
	$Cool_Music.play()
	register_win()
...
func _on_timer_timeout() -> void:
	$Explosions.show()
	get_node("Bomb").remove_child(get_node("Bomb/CollisionShape2D"))
	get_node("Bomb2").remove_child(get_node("Bomb2/CollisionShape2D"))
	get_node("Bomb4").remove_child(get_node("Bomb4/CollisionShape2D"))
	$Explosions.process_mode = Node.PROCESS_MODE_INHERIT
