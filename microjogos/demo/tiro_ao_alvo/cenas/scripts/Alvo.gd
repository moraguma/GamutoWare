...
func _ready():
	...
	position = Vector2(960, 540) + Vector2(0, -1).rotated(randf_range(-PI, PI)) * (RADIUS + randf_range(-RADIUS_VAR, RADIUS_VAR))
	...
	velocity = (Vector2(960, 540) - position).normalized().rotated(randf_range(-MAX_ANGLE, MAX_ANGLE)) * (SPEED + randf_range(-SPEED_VAR, SPEED_VAR))
